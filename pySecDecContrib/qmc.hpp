...
#endif
#ifndef QMC_FITFUNCTIONS_CNF_H
#define QMC_FITFUNCTIONS_CNF_H
...
namespace integrators
{
    namespace fitfunctions
    {
        namespace cnf_detail
        {
            ...
            // Serialises worker threads within each integrand.
            inline std::mutex& flow_batch_mutex()
            {
                static std::mutex m;
                return m;
            }
            ...
            inline void map_shm(const char* name, void* addr)
            {
                ...
                struct stat st;
                if (fstat(fd, &st) == -1) { std::perror(name); std::exit(1); }
            ...
            inline void ensure_shm_mapped()
            {
                ...
                std::call_once(flag, []() {
                    const char* ctrl_name = std::getenv("QMC_CTRL_NAME");
        ...
        struct CnfTransform
        {
            I f;
            const U number_of_integration_variables;
            std::shared_ptr<std::unique_lock<std::mutex>> integrand_lock;

            CnfTransform(const I& f) : f(f), number_of_integration_variables(f.number_of_integration_variables)
            {
                cnf_detail::ensure_shm_mapped();
            }

            template<typename Qmc_t>
            void fit(Qmc_t& qmc, I& func)
            {
                static_assert(sizeof(D) == 8, "fitfunctions::Cnf only supports double");

                if (!qmc.batching) {
                    qmc.logger << "warning: fitfunction='cnf' requires batching; enabling batching" << std::endl;
                    qmc.batching = true;
                }

                // Lives until all workers for this integrand are finished.
                integrand_lock = std::make_shared<std::unique_lock<std::mutex>>(cnf_detail::integrand_mutex());

                char* cnf_ctrl = cnf_detail::ctrl_addr;
                char* cnf_data = cnf_detail::data_addr;

                const U ndim = func.number_of_integration_variables;
                reinterpret_cast<long*>(cnf_ctrl)[1] = ndim;
                cnf_ctrl[0] = 2;

                while (cnf_ctrl[0] == 2) usleep(10000);
                if (cnf_ctrl[0] != 3) throw std::runtime_error("qmc_ctrl: invalid state");

                while (true) {
                    const long npoints = reinterpret_cast<long*>(cnf_ctrl)[1];
                    const double* points = reinterpret_cast<const double*>(cnf_data);
                    std::complex<double>* evals = reinterpret_cast<std::complex<double>*>(cnf_data);
                    std::vector<std::complex<double>> tmp(npoints);
                    for (long i = 0; i < npoints; i++) {
                        tmp[i] = func(const_cast<double*>(&points[i * ndim]));
                    }
                    std::memcpy(evals, tmp.data(), npoints * sizeof(std::complex<double>));
                    cnf_ctrl[0] = 4;

                    while (cnf_ctrl[0] == 4) usleep(10000);
                    if (cnf_ctrl[0] == 5) break;
                    if (cnf_ctrl[0] != 3) throw std::runtime_error("qmc_ctrl: invalid state");
                }
            }
            ...
            auto operator()(D* x) -> decltype(f(x))
            {
                throw std::logic_error("fitfunctions::Cnf single-point operator() is not supported");
            }
            ...
            void operator()(D* x, decltype(f(x))* res, U count)
            {
                ...
                std::vector<double> flowed(count * (ndim + 1));

                {
                    std::lock_guard<std::mutex> lock(cnf_detail::flow_batch_mutex());
                    ...
                    while (cnf_ctrl[0] == 6) usleep(10000);
                    if (cnf_ctrl[0] != 5) throw std::runtime_error("qmc_ctrl: invalid state");

                    std::memcpy(flowed.data(), cnf_data, count * (ndim + 1) * sizeof(double));
                }

                for (U i = 0; i < count; i++) {
                    res[i] = f(&flowed[i * (ndim + 1) + 1]) / flowed[i * (ndim + 1)];
                }
            }
        };
        ...
        struct CnfImpl
        {
            using function_t = CnfFunction<D>;
            using jacobian_t = std::nullptr_t;
            using hessian_t = std::nullptr_t;
            using transform_t = CnfTransform<I,D,M>;
        };

        struct Cnf
        {
            template<typename I, typename D, U M> using type = CnfImpl<I, D, M>;
        };
    };
};
...
// Periodizing Transforms
 ...
 namespace integrators
 {
    ...
    namespace core
    {
        template<typename Transform, typename Qmc_t, typename I, typename = void>
        struct has_custom_fit : std::false_type {};
        template<typename Transform, typename Qmc_t, typename I>
        struct has_custom_fit<Transform, Qmc_t, I, std::void_t<decltype(std::declval<Transform&>().fit(std::declval<Qmc_t&>(), std::declval<I&>()))>> : std::true_type {};
    }
    ...
    typename F<I,D,M>::transform_t Qmc<T,D,M,P,F,G,H>::fit(I& func)
    {
        ...
        if constexpr (core::has_custom_fit<typename F<I,D,M>::transform_t, Qmc<T,D,M,P,F,G,H>, I>::value) {
            fit_function_transform.fit(*this, func);
            return fit_function_transform;
        } else if (fit_function.num_parameters <= 0) {
            return fit_function_transform;
        } else {
             std::vector<D> x,y;
             std::vector<std::vector<D>> fit_parameters;
